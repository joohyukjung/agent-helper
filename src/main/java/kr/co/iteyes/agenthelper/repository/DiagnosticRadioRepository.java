...
public interface DiagnosticRadioRepository extends JpaRepository<DiagnosticRadio, DiagnosticRadioId> {
    @Query("select d from DiagnosticRadio d where d.patId = :patientId and d.rcptNo = :rcptNo")
    List<DiagnosticRadio> findAllResource(String patientId, BigInteger rcptNo);
}
