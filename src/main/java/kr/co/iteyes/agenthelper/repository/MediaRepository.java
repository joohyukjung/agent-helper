...
public interface MediaRepository extends JpaRepository<Media, MediaId> {
    @Query("select m from Media m where m.patId = :patientId and m.mediBgngYmd between :startDate and :endDate")
    List<Media> findAllResource(String patientId, String startDate, String endDate);
}
